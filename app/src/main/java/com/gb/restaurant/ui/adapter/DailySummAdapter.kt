...
class DailySummAdapter(val mContext:Context, var viewModel: AddTipsViewModel) : RecyclerView.Adapter<DailySummAdapter.OrderViewHolder>() {
    ...
    override fun getItemCount(): Int {
       return viewModel.getDailySummSize()
    }

    override fun onBindViewHolder(holder: OrderViewHolder, position: Int) {
        viewModel.getDailySummAt(position)?.let {
            holder.apply {
                    keyText.text = "${it.key}"
    ...
    inner class OrderViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
        ...
        val layout by lazy { itemView.findViewById(R.id.summary_layout) as LinearLayout }
